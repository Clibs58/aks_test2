...
/* ========= REUSABLE GRID (desktop full view) ========= */
...
export function Mentors() {
  ...
  useEffect(() => {
    const updateDrag = () => {
      if (!swipeRef.current || !containerRef.current) return;
      const fullWidth = swipeRef.current.scrollWidth;
      const containerWidth = containerRef.current.offsetWidth;
      const newDrag = fullWidth - containerWidth;
      setMaxDrag(newDrag > 0 ? newDrag : 0);
    };
  ...
  return (
    <section id="mentors" className="relative py-32 bg-black overflow-hidden">
      ...
      {/* ===== MOBILE SWIPE (fixed) ===== */}
      {!showAll && (
        <div className="md:hidden relative mb-16 px-6 overflow-hidden" ref={containerRef}>
          <motion.div
